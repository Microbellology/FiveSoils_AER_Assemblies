...
mkdir 09_Tu_musa_mapping

bwa mem /QRISdata/Q0775/belle/01_core/01_5soils_AER/02_novaseq/Musa_spp.fa /QRISdata/Q0775/belle/01_core/01_5soils_AER/02_novaseq/Tu/01_reads/Tu_R1.fastq /QRISdata/Q0775/belle/01_core/01_5soils_AER/02_novaseq/Tu/01_reads/Tu_R2.fastq > 09_Tu_musa_mapping/Tu_aln-pe.sam

cp -r 09_Tu_musa_mapping /QRISdata/Q0775/belle/01_core/01_5soils_AER/02_novaseq/Tu/'
...
samtools view -bS /QRISdata/Q0775/belle/01_core/01_5soils_AER/02_novaseq/Tu/09_Tu_musa_mapping/Tu_aln-pe.sam > sam/Tu_aln-pe.bam
#rm Tu_aln-pe.sam

##sort bam by read name (moves paired reads adjacent to each other), then remove unsorted bam to save space
samtools sort -n -m 8G -@ 10 sam/Tu_aln-pe.bam -o sam/Tu_aln-pe_sorted.bam
#rm Tu_aln-pe.bam

##subselect unmapped reads, then remove sorted bam
samtools view -b -f 12 -F 256 sam/Tu_aln-pe_sorted.bam > sam/Tu_aln-pe_unmapped.bam
#rm Tu_aln-pe_sorted.bam

##sort bam by read name again, then remove unsorted bam
samtools sort -n -m 8G -@ 10 sam/Tu_aln-pe_unmapped.bam -o sam/Tu_aln-pe_unmapped_sorted.bam
#rm Tu_aln-pe_unmapped.bam

#Extract unmapped read pairs, then remove bam
samtools fastq -@ 10 sam/Tu_aln-pe_unmapped_sorted.bam -1 sam/Tu_cleaned_R1.fq.gz -2 sam/Tu_cleaned_R2.fq.gz -0 /dev/null -s /dev/null -n
#rm ref_genome_sample_1_unmapped_sorted.bam

cp -r sam/ /QRISdata/Q0775/belle/01_core/01_5soils_AER/02_novaseq/Tu/09_Tu_musa_mapping/'
...
mkdir Tu_spades
metaspades.py -1 /QRISdata/Q0775/belle/01_core/01_5soils_AER/02_novaseq/Tu/09_Tu_musa_mapping/Tu_cleaned_R1.fq.gz -2/QRISdata/Q0775/belle/01_core/01_5soils_AER/02_novaseq/Tu/09_Tu_musa_mapping/Tu_cleaned_R2.fq.gz -o Tu_spades/ -t 24 -m 1975

cp -r Tu_spades/ /QRISdata/Q0775/belle/01_core/01_5soils_AER/02_novaseq/Tu/'

#submit job
qsub Tu_spades.pbs

$Opera_assembly
#using musa free reads. See the beginning of NB 4.2 for details
...
#run your commands
OPERA-MS.pl --long-read-mapper blasr --no-polishing --no-strain-clustering --short-read1 Tu_cleaned_R1.fq.gz --short-read2 Tu_cleaned_R2.fq.gz --contig-file contigs.fasta --long-read Tu_long_read.fastq --out-dir 03_opera/ --num-processors 10
...
cp spades.contigs.min1500.fasta /safs-data02/dennislab/Public/belle/5soils/Tu/03_opera/
...
#submit Job
sbatch -c 24 --mem=250GB --partition=safs --priority=999999999 --time=365-0:00:00 --no-kill --job-name=Tu_sopera 3_opera.sh
 #job id

sbatch -c 24 --mem=250GB --partition=safs --time=365-0:00:00 --no-kill --job-name=Tu_sopera 3_opera.sh
10871145 #job id

#Still didn't work, ran the above script manually via the compute nodes in a screen
...
#move to input file location
cd /safs-data02/dennislab/Public/belle/5soils/Tu/03_spades_assembly/

#copy your program input files to the scratch directory
cp 01_opera/spades.contigs.min1500.fasta /safs-data01/uqbclar8/Tu/
...
for i in `ls *_R1.fq.gz`; do
  bwa mem -t "24" spades.contigs.min1500.fasta ${i} ${i%R1.fq.gz}R2.fq.gz | samtools view -b -o ${i%_cleaned_R1.fq.gz}mapped.bam
...
#binning
metabat2 --inFile spades.contigs.min1500.fasta --outFile 04_bins/bin --abdFile Tu_contigs_depth.txt --minContig=1500

#copy required files to safs server
cp *sorted.bam /safs-data02/dennislab/Public/belle/5soils/Tu/03_spades_assembly/02_bam/
cp 04_bins/*fa /safs-data02/dennislab/Public/belle/5soils/Tu/03_spades_assembly/03_bins/
...
sbatch -c 24 --mem=250GB --partition=safs --time=365-0:00:00 --no-kill --job-name=Tu_Sbins 4_binning.sh
11038526

$Check_and_Polishing

#Made and error with the first checkM, line 227.  Had to rerun from this point
{#CheckM script
'#!/usr/bin/env bash
. /safs-data02/dennislab/sw/.config/software_defs
...
sbatch -c 15 --mem=500GB --partition=safs --priority=999999999 --time=365-0:00:00 --no-kill --job-name=Tu_Spilon 6_pilon.sh
Submitted batch job 11198722
...
sbatch -c 10 --mem=100GB --partition=safs --priority=999999999 --time=365-0:00:00 --no-kill --job-name=Tu_ScheckM 5_checkM.sh
Submitted batch job 11226071
}
...
sbatch -c 10 --mem=100GB --partition=safs --priority=999999999 --time=365-0:00:00 --no-kill --job-name=Tu_checkM 5_checkM.sh
Submitted batch job 11227510

#rerun pilon using script above

sbatch -c 15 --mem=500GB --partition=safs --priority=999999999 --time=365-0:00:00 --no-kill --job-name=Tu_Spilon 6_pilon.sh
Submitted batch job 11229834

#rerun checm on polished bins
'#!/usr/bin/env bash
. /safs-data02/dennislab/sw/.config/software_defs
...
sbatch -c 10 --mem=100GB --partition=safs --priority=999999999 --time=365-0:0000 --no-kill --job-name=Tu_checkM 5_checkM.sh
Submitted batch job 11267254
