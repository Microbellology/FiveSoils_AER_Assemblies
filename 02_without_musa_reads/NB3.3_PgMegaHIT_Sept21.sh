...
$Opera_assembly
#using musa free reads. See the beginning of NB 3.2 for details
...
#run your commands
OPERA-MS.pl --long-read-mapper blasr --no-polishing --no-strain-clustering --short-read1 Pg_cleaned_R1.fq.gz --short-read2 Pg_cleaned_R2.fq.gz --long-read Pg_long_read.fastq --out-dir 03_opera/ --num-processors 48
...
cp contigs.min1500.fasta /safs-data02/dennislab/Public/Belle/5soils/Pg/03_opera/
...
sbatch -c 48 --mem=1450GB --partition=safs --time=365-0:00:00 --no-kill --job-name=Pg_opera 3_opera.sh
10645367 #job number
...
#move to input file location
cd /safs-data02/dennislab/Public/belle/5soils/Pg/02_megahit_assembly/01_opera/

#copy your program input files to the scratch directory
cp metahit.contigs.min1500.fasta /safs-data01/uqbclar8/Pg/
...
#index contigs file
bwa index metahit.contigs.min1500.fasta

#map reads to contiq files and then sort by read name (puts f and r pairs next to each other)
for i in `ls *_R1.fq.gz`; do
  bwa mem -t "24" metahit.contigs.min1500.fasta ${i} ${i%R1.fq.gz}R2.fq.gz | samtools view -b -o ${i%_cleaned_R1.fq.gz}mapped.bam
...
#binning
metabat2 --inFile metahit.contigs.min1500.fasta --outFile 04_bins/bin --abdFile Pg_contigs_depth.txt --minContig=1500

#copy required files to safs server
cp *sorted.bam /safs-data02/dennislab/Public/belle/5soils/Pg/02_megahit_assembly/02_bam/
cp 04_bins/*fa /safs-data02/dennislab/Public/belle/5soils/Pg/02_megahit_assembly/03_bins/

#remove items from scratch drive
cd ../
rm -r Pg'

 sbatch -c 24 --mem=250GB --partition=safs --time=365-0:00:00 --no-kill --job-name=Pg_Mbins 4_binning.sh
 11038525
...
#run as sbatch
sbatch -c 10 --mem=100GB --partition=safs --priority=999999999 --time=365-0:00:00 --no-kill --job-name=Pg_checkM 5.checkM.sh
Submitted batch job 11058258
...
sbatch -c 15 --mem=500GB --partition=safs --priority=999999999 --time=365-0:00:00 --no-kill --job-name=Pg_pilon 6_pilon.sh
Submitted batch job 11195715
