...
#cp 02_spades/contigs.fasta /safs-data01/uqbclar8/To/
#move to the scratch directory
cd /safs-data01/uqbclar8/To/
mkdir 03_opera

#run your commands
OPERA-MS.pl --long-read-mapper blasr --no-polishing --no-strain-clustering --short-read1 To_cleaned_R1.fq.gz --short-read2 To_cleaned_R2.fq.gz --long-read To_long_read.fastq --out-dir 03_opera/ --num-processors 48
...
cp contigs.min1500.fasta /safs-data02/dennislab/Public/Belle/5soils/To/03_opera/
...
sbatch -c 48 --mem=1450GB --partition=safs --time=365-0:00:00 --no-kill --job-name=To_opera 3_opera.sh
10645368 #job number
...
#copy your program input files to the scratch directory
cp 03_opera/contigs.min1500.fasta /safs-data01/uqbclar8/To/
...
#binning
metabat2 --inFile contigs.min1500.fasta --outFile 04_bins/bin --abdFile To_contigs_depth.txt --minContig=1500
###
#copy required files to safs server
mv *sorted.bam bam_files/
mv bam_files/ 04_bins/
cp -r 04_bins/ /safs-data02/dennislab/Public/belle/5soils/To/
...
#Run manually on a compute node with 10 cores and 100mem
...
#submit as sbatch
sbatch -c 10 --mem=100GB --partition=safs --priority=999999999 --time=365-0:00:00 --no-kill --job-name=To_checkM 5.checkM.sh
Submitted batch job 11058277
...
sbatch -c 15 --mem=500GB --partition=safs --priority=999999999 --time=365-0:00:00 --no-kill --job-name=To_pilon 6_pilon.sh
Submitted batch job 11195725
