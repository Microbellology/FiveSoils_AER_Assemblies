...
$Opera_assembly #15.9.21
#using musa free reads. See the beginning of NB 2.2 for details
...
#run your commands
OPERA-MS.pl --long-read-mapper blasr --no-polishing --no-strain-clustering --short-read1 Li_cleaned_R1.fq.gz --short-read2 Li_cleaned_R2.fq.gz --long-read Li_long_read.fastq --out-dir 03_opera/ --num-processors 48
...
cp contigs.min1500.fasta /safs-data02/dennislab/Public/Belle/5soils/Li/03_opera/
...
#submit job
sbatch -c 48 --mem=1450GB --partition=safs --priority=999999999 --time=365-0:00:00 --no-kill --job-name=Li_opera 3_opera.sh
10645355 #job number

10652080 #Job ID after adding gunzip step
...
#copy your program input files to the scratch directory
cp 03_opera/contigs.min1500.fasta /safs-data01/uqbclar8/Li/
...
#binning
metabat2 --inFile contigs.min1500.fasta --outFile 04_bins/bin --abdFile Li_contigs_depth.txt --minContig=1500

#copy required files to safs server
mv *sorted.bam bam_files/
mv bam_files/ 04_bins/
cp -r 04_bins/ /safs-data02/dennislab/Public/belle/5soils/Li/

#remove items from scratch drive
cd ../
rm -r Li'

#run manually in a screen with 10 cores and 100mem
...
#run job as sbatch
sbatch -c 10 --mem=100GB --partition=safs --priority=999999999 --time=365-0:00:00 --no-kill --job-name=Li_checkM 5.checkM.sh
11058248 #jod ID
...
'
#run as Sbatch
sbatch -c 15 --mem=500GB --partition=safs --priority=999999999 --time=365-0:00:00 --no-kill --job-name=Li_pilon 6_pilon.sh
Submitted batch job 11166250
